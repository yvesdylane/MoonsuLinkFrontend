...
import { api, type IssueItem } from '@/services/api'
...
function TypeBadge({ type }: { type: string }) {
  const styles: Record<string, string> = {
    ...
    pest: 'bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400',
    soil: 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400',
    weather: 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400',
    technique: 'bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400',
...
export default function IssuesPage() {
  ...
  const [viewItem, setViewItem] = useState<IssueItem | null>(null)
  ...
  }, [page, limit, statusFilter, typeFilter, regionFilter])

  useEffect(() => {
    loadIssues() // eslint-disable-line react-hooks/set-state-in-effect
  }, [loadIssues])
  ...
  const toggleStatus = async () => {
    if (!viewItem) return
    const newStatus = viewItem.status === 'open' ? 'resolved' : 'open'
    setUpdating(true)
    setError('')
    try {
      await api.updateIssue(viewItem.id, { status: newStatus })
      setSuccessMsg(`Issue marked as "${newStatus}"`)
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 dark:border-zinc-700 bg-zinc-50 dark:bg-primary-dark">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 hidden sm:table-cell">Author</th>
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-700">
                {issues.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-50 dark:hover:bg-primary-dark transition-colors">
                    <td className="px-4 py-3"><TypeBadge type={item.issue_type} /></td>
                    <td className="px-4 py-3 font-medium text-primary dark:text-white max-w-[200px] truncate">
                      {item.title}
                    </td>
                    <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400 hidden sm:table-cell text-xs">{item.author_name}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-primary-light p-6 shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-semibold text-primary dark:text-white">Issue #{viewItem.id}</h2>
            ...
            {/* Issue info */}
            <div className="mb-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mb-3">Issue</p>
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                <Field label="Type"><TypeBadge type={viewItem.issue_type} /></Field>
            ...
            <hr className="border-zinc-100 dark:border-zinc-700 mb-5" />
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mb-3">Actions</p>
              <button
              ...
              >
                {updating ? 'Updating...' : viewItem.status === 'open' ? 'Mark as Resolved' : 'Reopen Issue'}
              </button>
            </div>
